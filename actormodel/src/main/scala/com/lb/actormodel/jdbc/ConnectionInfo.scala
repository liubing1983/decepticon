...
class ConnectionInfo {
  ...
  def getConn: Connection = {
    if (conn == null) {
      println("申请Connection连接!!!" + "--"+Thread.currentThread().getName)
      conn  = cpds.getConnectionPoolDataSource().getPooledConnection().getConnection()
      conn
    } else {
      print("清闲的：" +  cpds.getNumIdleConnections)
      print("-忙碌的：" + cpds.getNumBusyConnections)
      print("-所有的：" + cpds.getNumConnections)
